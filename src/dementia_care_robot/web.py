...
<div class="voice-panel"><button type="button" id="talkButton" data-generated="{str(app.generated_responses).lower()}">Hold to speak</button><p id="voiceStatus" role="status" aria-live="assertive">{"Press and hold while speaking, then release to send." if app.generated_responses else "Offline testing uses your browser's speech recognition. Press and hold to speak."}</p>{'' if app.generated_responses else '<p class="helper">No OpenAI charges. Depending on the browser and device, speech may be processed by the browser vendor.</p>'}</div>
...
const usesServerTranscription = talk.dataset.generated === 'true';
